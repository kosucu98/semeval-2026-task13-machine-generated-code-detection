...
class LlamaTrainerTaskC:
    def __init__(self, max_length=256, model_dir=None, seed=42):
        ...
        # original_label(int) -> internal_id(int)
        self.label2id = None
        self.id2label = None
    ...
    def load_and_prepare_data(self, train_parquet, val_parquet):
        ...
        if TRAIN_LIMIT is not None:
            ...
            print(f"[data] Using RANDOM {n} training samples (seed={self.seed}).")
            df = df.sample(n=n, random_state=self.seed).reset_index(drop=True).copy()
        else:
            print(f"[data] Using ALL training samples: {len(df)}")

        if VAL_LIMIT is not None:
            n = min(int(VAL_LIMIT), len(val_df))
            print(f"[data] Using RANDOM {n} validation samples (seed={self.seed + 1}).")
            val_df = val_df.sample(n=n, random_state=self.seed + 1).reset_index(drop=True).copy()
        else:
            print(f"[data] Using ALL validation samples: {len(val_df)}")

        # Label remapping is useful for Task C because labels may not always be 0..K-1
        unique_train_labels = sorted(df["label"].unique().tolist())
        self.label2id = {label: idx for idx, label in enumerate(unique_train_labels)}
        self.id2label = {idx: label for label, idx in self.label2id.items()}
        self.num_labels = len(self.label2id)

        unseen_labels = sorted(set(val_df["label"].unique().tolist()) - set(unique_train_labels))
        if unseen_labels:
            raise ValueError(f"Validation has labels not present in training: {unseen_labels}")

        df["label"] = df["label"].map(self.label2id).astype(int)
        val_df["label"] = val_df["label"].map(self.label2id).astype(int)

        print("[data] unique train labels (original):", unique_train_labels)
    ...
    def prepare_datasets(self, df, val_df):
        ...
        train_cache_dir = os.path.join(
            cache_root,
            f"train_len{self.max_length}_seed{self.seed}_n{len(df)}",
        )
        val_cache_dir = os.path.join(
            cache_root,
            f"val_len{self.max_length}_seed{self.seed + 1}_n{len(val_df)}",
        )
    ...
    @torch.no_grad()
    def predict_test_to_csv(self, parquet_path, output_path, batch_size=16, device=None):
        """
        Streaming inference over a parquet file with columns ['ID', 'code'] or ['id', 'code'].
        Writes CSV in the format: ID,label

        For Task C, predicted labels are converted back to the original label IDs.
        """
        if _is_ddp() and not _is_main_process():
            return
...
def main():
    ...
    parser.add_argument("--train_parquet", type=str, required=True, help="Path to Task C training parquet file.")
    parser.add_argument("--val_parquet", type=str, required=True, help="Path to Task C validation parquet file.")
    ...
    parser.add_argument(
        "--predictions_csv",
        type=str,
        default="submission_taskC_llama.csv",
